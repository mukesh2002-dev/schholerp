...
export function getNavForRole(role: Role | StaffRole | undefined): RoleNavGroup[] {
  ...
  const staffRole = (["SUPER_ADMIN", "PRINCIPAL", "ACCOUNTANT", "HR_MANAGER"] as const).includes(
    role as StaffRole
  )
    ? (role as StaffRole)
    : "SUPER_ADMIN";
  ...
  return ROLE_NAV_GROUPS.map((group) => ({
    ...group,
    items: group.items.filter((item) => !item.allowedRoles || item.allowedRoles.includes(staffRole)),
