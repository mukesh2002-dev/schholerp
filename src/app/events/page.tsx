      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-1">
            Manage and view all school events, activities, and holidays.
          </p>
        </div>
      </div>
                    ...
                    <div className="space-y-0.5">
                      {dayEvents.slice(0, 2).map((evt) => (
                        <Link
                          key={evt.id}
                          href={`/events/${evt.id}`}
                          className="block rounded px-1 py-0.5 text-[10px] font-medium truncate hover:bg-primary/10 transition-colors"
                          style={{
                            backgroundColor: `var(--${categoryColor[evt.category]}-500, var(--primary))`,
                            color: "white",
                          }}
                        >
                          {evt.title}
