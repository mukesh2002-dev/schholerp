...
export default function AnnouncementsPage() {
  ...
  const [announcements, setAnnouncements] = useState(() => mockDb.getAnnouncements(activeBranchId));
  ...
  const refreshList = () => {
    setAnnouncements(mockDb.getAnnouncements(activeBranchId));
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-1">
            View and manage school-wide announcements, notices, and important updates.
          </p>
        </div>
      </div>
      ...
      {filteredAnnouncements.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredAnnouncements.map((ann) => (
            <Link key={ann.id} href={`/announcements/${ann.id}`}>
              <Card className="border-border/80 shadow-xs hover:shadow-md transition-shadow cursor-pointer">
