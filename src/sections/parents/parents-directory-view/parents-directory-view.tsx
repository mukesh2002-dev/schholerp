...
import { Search, CalendarCheck, DollarSign, Medal, Bell, BookOpen } from "lucide-react";

export function ParentsDirectoryView() {
  ...
  const invoices = useMemo(
    () => (selected ? (mockDb.getInvoices() || []).filter((i) => i.studentId === selected.id) : []),
    [selected]
  );
  ...
  const attendance = useMemo(
    () =>
      selected
        ? (mockDb.getAttendanceRecords(undefined, "STUDENT") || [])
  ...
  const results = useMemo(
    () => (selected ? (mockDb.getResults() || []).filter((r) => r.studentId === selected.id) : []),
    [selected]
  );

  const notices = useMemo(
    () => (mockDb.getNotices(activeBranchId) || []).slice(0, 5),
    [activeBranchId]
  );

  const announcements = useMemo(
    () => (mockDb.getAnnouncements(activeBranchId) || []).slice(0, 3),
    [activeBranchId]
  );
    ...
    <div className="space-y-6">
        ...
        <CardContent className="p-4 flex flex-col md:flex-row gap-4 items-center">
          <div className="relative flex-1 max-w-sm w-full">
            ...
            <Input
              ...
              placeholder="Search ward by name or roll..."
          ...
          <div className="flex gap-2 flex-wrap">
            {filtered.slice(0, 6).map((s) => (
              ...
              >
                ...
                <span className="text-[10px] opacity-70">{s.rollNumber}</span>
              </button>
            ))}
          </div>
        </CardContent>
      ...
      <Card className="border-primary/20 bg-gradient-to-r from-blue-500/5 via-indigo-500/5 to-purple-500/5">
        <CardContent className="p-5 flex flex-col md:flex-row gap-5 items-start">
          <img
            src={selected.avatar || "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=100"}
            alt={selected.fullName}
            className="h-20 w-20 rounded-2xl object-cover ring-2 ring-border shrink-0"
          />
          <div className="flex-1 min-w-0">
            <h3 className="text-lg font-bold">
              {selected.fullName}{" "}
              <span className="text-sm font-normal text-muted-foreground">({selected.rollNumber})</span>
            </h3>
            <p className="text-xs text-muted-foreground">
              {selected.className} &bull; {selected.sectionName} &bull; {selected.branchName}
            </p>
            <div className="flex flex-wrap gap-2 mt-2">
              <Badge variant="outline" className="text-[10px]">
                Guardian: {selected.guardian?.name || "Guardian"} ({selected.guardian?.relation || "Parent"})
              </Badge>
              <Badge variant="secondary" className="text-[10px]">
                {selected.status}
              </Badge>
              <Badge variant="outline" className="text-[10px]">
                {selected.guardian?.phone || "N/A"}
              </Badge>
            </div>
            <div className="grid grid-cols-3 gap-3 mt-4">
              <div className="p-3 rounded-xl bg-card border text-center">
                ...
                <span className="text-lg font-bold text-emerald-600">
                  {selected.attendanceSummary?.attendanceRate || 95}%
                </span>
                <span className="text-[10px] text-muted-foreground">
                  {selected.attendanceSummary?.presentDays || 0}/{selected.attendanceSummary?.totalDays || 0} days
                </span>
              </div>
              <div className="p-3 rounded-xl bg-card border text-center">
                <span className="text-xs text-muted-foreground block">Fees Due</span>
                <span className="text-lg font-bold text-amber-600">
                  {formatCurrency(selected.feeSummary?.totalPending || 0)}
                </span>
                <span className="text-[10px] text-muted-foreground">
                  {selected.feeSummary?.status || "UP_TO_DATE"}
                </span>
              </div>
              <div className="p-3 rounded-xl bg-card border text-center">
                <span className="text-xs text-muted-foreground block">GPA / Rank</span>
                <span className="text-lg font-bold text-blue-600">
                  {selected.academicHistory?.[0]?.gpa || "3.8"}
                </span>
                <span className="text-[10px] text-muted-foreground">
                  {selected.academicHistory?.[0]?.percentage || 90}%
                </span>
              </div>
            </div>
          </div>
      ...
      <Tabs defaultValue="attendance" className="space-y-4">
        ...
        <TabsContent value="attendance" className="space-y-4">
          ...
          <Card>
            <CardContent className="p-4 text-xs text-muted-foreground flex items-center gap-2">
              ...
              Attendance synced via biometric devices &bull; Daily updates reflected live here.
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="fees" className="space-y-4">
          ...
          {invoices.length === 0 && (
            <div className="text-center py-6 text-muted-foreground text-sm">
              No invoices found for this ward.
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="results" className="space-y-4">
          {results.length === 0 ? (
            ...
            <div className="grid gap-4">
              {results.map((res) => (
                <Card key={res.id} className="border-border/80">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm flex justify-between">
                      {res.examTypeName} &bull; {res.academicYear}
                      <Badge variant={res.status === "PASS" ? "success" : "destructive"} className="text-[10px]">
                        {res.status}
                      </Badge>
                    </CardTitle>
                    <p className="text-xs text-muted-foreground">
                      {res.className} {res.sectionName} &bull; Rank {res.rank}/{res.totalStudents} &bull; GPA {res.gpa}
                    </p>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    <div className="flex justify-between text-xs font-bold border-b pb-2">
                      <span>
                        {res.marksObtained}/{res.totalMarks} &bull; {res.percentage}% &bull; {res.overallGrade}
                      </span>
                      <span className="text-emerald-600">{res.status}</span>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
                      {(res.subjects || []).map((sub) => (
                        <div key={sub.subjectId} className="p-2 rounded-lg bg-muted/40 border flex justify-between">
                          <span>{sub.subjectName}</span>
                          <span className="font-bold">
                            {sub.marksObtained}/{sub.totalMarks} &bull; {sub.grade}
                          </span>
                        </div>
                      ))}
                    </div>
          ...
          {(selected.academicHistory || []).length > 0 && (
            <Card>
              ...
              <CardContent className="space-y-2">
                {selected.academicHistory.map((h) => (
                  <div key={h.term} className="flex justify-between text-xs p-2 rounded-lg bg-muted/40 border">
                    <span>
                      {h.term} &bull; {h.grade}
                    </span>
                    <span className="font-semibold">
                      {h.percentage}% &bull; GPA {h.gpa} &bull; Rank {h.rank}
                    </span>
                  </div>
                ))}
              </CardContent>
        ...
        <TabsContent value="notices" className="space-y-4">
          <div className="grid gap-3">
            {notices.map((n) => (
              <Card key={n.id} className="border-border/80">
                <CardContent className="p-4">
                  ...
                  <p className="text-[11px] text-muted-foreground mt-2">
                    {formatDate(n.date)} &bull; {n.category} &bull; {n.branchName}
                  </p>
                </CardContent>
              </Card>
            ))}
            {announcements.map((a) => (
              <Card key={a.id} className="border-border/80 bg-blue-500/5">
                <CardContent className="p-4">
                  <h4 className="font-semibold text-sm">{a.title}</h4>
                  <p className="text-xs text-muted-foreground mt-1">{a.summary}</p>
                  <p className="text-[11px] text-muted-foreground mt-2">
                    {formatDate(a.publishDate)} &bull; {a.branchName}
                  </p>
                </CardContent>
              </Card>
            ))}
